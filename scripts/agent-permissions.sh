...
#                            and .work/; DISCOVERY.md is readable, not writable
#
...
if [[ -e "DISCOVERY.md" ]]; then
  if $CHECK; then
    if [[ "$(group_of DISCOVERY.md)" == "$AGENT_GROUP" && \
          "$(mode_of DISCOVERY.md)" == "640" ]]; then
      note "DISCOVERY.md 640 ${AGENT_GROUP}"
    else
      fail "DISCOVERY.md is $(mode_of DISCOVERY.md) $(group_of DISCOVERY.md), want 640 ${AGENT_GROUP}"
    fi
  else
    sudo chgrp "$AGENT_GROUP" DISCOVERY.md
